...
接続には必要に応じてサブプロトコル名や認証トークンをヘッダに含めます。サーバがサポートしない場合、初期ハンドシェイクで 4xx を返して接続を拒否することができます。
...
大量のイベントが発生する場合、サーバはバッファ上限を超えないようドロップや圧縮を行い、その旨をエラーフレームで通知できます。クライアントは欠損したイベントをリスト API などで再取得する戦略を持つべきです（SHOULD）。

## 6. ハートビートと切断

長時間の接続維持のため、サーバとクライアントは定期的に ping/pong などのハートビートを交換することが望まれます（SHOULD）。切断後はクライアントが再接続し、必要に応じて購読を再送します。

再接続時に前回の購読状態を復元するため、クライアントは購読リストや最後に受信したイベントのインデックスを保持しておくとよいでしょう。サーバが履歴再送をサポートする場合は、そのパラメータを明示します。

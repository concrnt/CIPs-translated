...
CIP-0で定義される"net.concrnt.core.resource"エンドポイントに対して、Reference DocumentのURIを指定してアクセスした場合、Concrntサーバーはreferenceフィールド内のhrefで指定されたURLを304リダイレクトレスポンスでクライアントに返却しなければならない(MUST)。

...
サブキーの導入により、メイン鍵をオフラインに保管しつつ、日常の署名操作をサブキーに分担できます。侵害時には revoke を公開することで迅速に無効化できます。
...
失効を公開した後も古いキャッシュが残る可能性があるため、クライアントは最新の `createdAt` を持つ revoke を優先し、再生攻撃を防ぎます。
...
複数の enact が存在する場合は最も新しいものを採用し、複数の revoke がある場合も最新のものを優先します。サブキーとメインキーの CSID/CCID は一致しないため、`author` が一致することを必ず確認してください。

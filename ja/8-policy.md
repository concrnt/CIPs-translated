...
Statement が dominant の場合、条件が成立した時点で評価を終了し、その結果を採用します。defaultOnTrue と defaultOnFalse は条件が真または偽のときに採用する既定の判定を示します。

## 5. アクション

代表的なアクションには、レコードの読み取りや更新、削除、関連付け、コレクション配布などがあります。実装は必要に応じてアクション名を拡張できますが、意味を明確にし一貫した名前を用いるべきです（SHOULD）。

例として、`record.read`, `record.update`, `record.delete`, `record.associate`, `record.retract`, `collection.distribute`, `collection.retract` などが含まれます。アクションはオブジェクトの種類や操作の粒度に応じて整理します。
...
`LoadSelf` はポリシーがアタッチされたリソースの Document から値を読み出し、`LoadResource` は評価中のリソースから値を読み出します。`IsCCID`、`IsCSID`、`IsCKID` は文字列がそれぞれの形式として妥当かを検査します。`RequesterHasTag` や `RequesterDomainHasTag` は呼び出しユーザーや所属ドメインに付与されたタグを評価します。
...
評価に必要な外部リソース（例: 呼び出しユーザーのタグ一覧）を取得できない場合のデフォルト動作も定義してください。多くの場合、取得失敗時は拒否を返すことが安全です。
...
評価結果や決定理由を監査ログに記録し、誤設定時に追跡できるようにすることも推奨されます。***

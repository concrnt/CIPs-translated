...
> **編集ノート:** 実装(`types.go:76`)ではDocumentの `policy` フィールドに
> `// CIP-8` というコメントが付いているが、現行のCIP番号体系では 12=Policy である。
> 実装側のコメント修正が必要 (CIP-1 §4 の編集ノートも参照)。
...
> **編集ノート:** allowlistを仕様でどこまで規定するか要検討。
> 現状は実装定義(in-code)であり、ポリシーの可搬性の観点では
> 「相互運用可能な最小セット」を仕様で規定する余地がある。
...
| `requester_domain` | 要求元エンティティの所属サーバー情報 |
| `parent` | 対象リソースの親リソース |
...
`requester.tag` は、サーバーがエンティティに付与したタグのカンマ区切り文字列である
(例: `"_admin,_invite"`。各タグは `key` または `key:value` 形式)。

> **編集ノート:** リファレンス実装は現在、評価時に `requester` / `self` /
> `params` / `globals` のみを設定しており、`requester_domain` と `parent` は
> 定義のみで未設定(常にnull)。仕様として残すか削除するか要判断。
> また `requester.tag` は文字列であるため `Contains` では直接判定できない。
> タグ文法の正式な定義をCIP-12に含めるかCIP-0へ移すかも要検討。
...
2. **仮想親(配布先)のポリシー** — 対象リソースが `distributes`(CIP-7)によって
   タイムライン等へ配布されている場合、その配布先リソースに関連付けられたポリシー。
   配布先を宣言しているリソースのレイヤーの直前に挿入される。
   これにより、タイムラインのポリシーがそこに配布される投稿を統制できる。
3. **対象リソース自身のポリシー** — 対象Documentの `policy` フィールドから解決されたもの。
4. **祖先のポリシー** — 対象リソースのキー階層(cckv URI のパス構造)を親方向に遡り、
   各階層のDocumentに関連付けられたポリシーを1階層1レイヤーとして順に積む。
...
> **編集ノート:** 弱い判定が「後勝ち」となるのはリファレンス実装
> (`EvaluateStack`)の挙動をそのまま仕様化したもの。スタック順は
> グローバル→対象リソース→祖先の順であるため、弱い判定については
> 祖先(ルート側)のポリシーが対象リソース自身のポリシーを上書きすることになる。
> これが意図された優先順位か要確認(直感的には「深い方が勝つ」が自然)。

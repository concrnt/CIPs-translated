...
Concrnt Core の目標は、CCID と公開鍵による暗号学的アイデンティティの確立、`cc://<CCID>/<key>` 形式のサーバ非依存な名前空間、所属先を署名付きで示す Affiliation の明示です。Core はあくまで最小の識別層とし、投稿やタイムラインなど上位の行動モデルは別の CIP で定義します。一方で、巨大インデックスサーバやコンテンツ配信の仕様、アプリケーション固有の署名形式やモデレーション言語は本仕様の範囲外とします。
...
CCURI が指すリソースの中身や MIME 形式、署名方式は Core では規定しません。クライアントは Core で得られるテンプレートを上位仕様のルールに従って解釈し、適切な Accept ヘッダや署名検証を行います。
...
鍵はクライアント側で生成・保持し、サーバに送信してはなりません（MUST NOT）。CCID の導出は公開鍵から一意に決まるため、秘密鍵の漏洩はアイデンティティの乗っ取りに直結します。
...
サーバは自身が直接管理しないエンティティについても、フェデレーションやキャッシュによって得た Affiliation を提供してもかまいません（MAY）。ただし、古い Affiliation への巻き戻しを防ぐため、より新しい `createdAt` を常に優先する必要があります。
